...
(function () {
    ...
    function getLessCompiler(lookupTable) {
        ...
        less.Parser.fileLoader = function (file, currentFileInfo, callback, env) {
            ...
            if (/{{.+}}/.test(file)) {
                // File path has '{{unit_name}}'.
                var startIndex = file.indexOf("{{");
                var endIndex = file.indexOf("}}", (startIndex + 2));
                var unit = lookupTable.units[file.substr((startIndex + 2), (endIndex - 2))];
                if (!unit) {
                    callback({
                        type: 'Unit',
                        message: "Unit '" + (file.substr(2, (endIndex - 2)))
                                 + "' mentioned in file '" + file + "' cannot be found."
                    });
                    return;
                }
                var relativePath = constants.DIRECTORY_APP_UNIT_PUBLIC + file.substr(endIndex + 2);
                lessFile = Utils.getFileInUiComponent(unit, "unit", relativePath, lookupTable);
                if (!lessFile) {
                    callback({
                        type: 'File',
                        message: "File '" + relativePath + "' cannot be found in unit '"
                                 + unit.fullName + "'."
                    });
                    return;
                }
