/**
 * Registers Handlebars helpers that are needed to render pages.
 * @param request {Object} current HTTP request
 * @param lookUpTable {{layouts: Object.<string, string>, units: Object.<string, {shortName:
 *     string, path: string, definition: Object}>, pushedUnits: Object.<string, string[]>}} lookup
 *     table
...
function registerHelpers(request, lookUpTable, configs) {
    ...
    function getUnitContextAndTemplate(unitModel, contextParams) {
        ...
        var unitPublicUri = "/" + contextParams.appName + constants.DIRECTORY_APP_UNIT_PUBLIC
                            + "/" + contextParams.unitFullName;
        var unitContext = {};
        var unitJsFile = new File(unitFilePath + ".js");
        if (unitJsFile.isExists()) {
            var script = require(unitJsFile.getPath());
            if (script.hasOwnProperty(constants.UNIT_JS_FUNCTION_ON_REQUEST)) {
                var jsContext = {
                    app: {
                ...
                unitContext = script[constants.UNIT_JS_FUNCTION_ON_REQUEST](jsContext);
            }
        }
