...
function registerHelpers(renderingData, lookupTable) {
    ...
    function isUnitProcessable(unit) {
        var isDisabled = Utils.parseBoolean(unit.definition[constants.UNIT_DEFINITION_DISABLE],
                                            false);
    ...
    function executeScript(uiComponent, uiComponentType, scriptData, scriptContext, lookupTable) {
        var unitScript = require(scriptData.scriptFilePath);
        unitScript.getFile = function (relativeFilePath) {
            Utils.getFileInUiComponent(uiComponent, uiComponentType, relativeFilePath, lookupTable);
        };
        var rv = unitScript[constants.UI_COMPONENT_JS_FUNCTION_ON_REQUEST](scriptContext);
        return (rv) ? rv : {};
    }
    ...
    function zoneHelper(zoneName, options) {
        ...
        var isOverride = Utils.parseBoolean(options.hash["override"], true);
    ...
    function resourceHelper(type, path, options) {
        ...
        mainZone.addResource(type, resourceProvider, (resourceProvider.fullName + path),
                             Utils.parseBoolean(options.hash["combine"], true));
        return "";
    }
    ...
    function defineZoneHelper(zoneName, options) {
        ...
        if (zonesStackSize == 0) {
            ...
            if (mainZone) {
                ...
                if (options.hash["scope"] == "protected") {
