...
function registerHelpers(renderingData, lookupTable, configs) {
    ...
    /**
     * Compares two UI Components by the 'index' values.
    ...
    function compareUiComponents(a, b) {
        // When making the lookup table, 'index' of a UI Component is assured to be unique.
        // So we don't have to check for indexes equality.
    ...
    function pageHelper(pageFullName, options) {
        ...
        if (templateFilePath) {
            var pageContent = readFile(page.templateFilePath);
            if (!pageContent) {
                var msg = "Cannot read template '" + page.templateFilePath + "' of page '"
                          + pageFullName + "'.";
                log.error(msg);
                throw new handlebarsEnvironment.Exception(msg);
            }
    ...
    function zoneHelper(zoneName, options) {
        ...
        if (runtimeData.currentPage) {
            ...
            if (unitsStackSize > 0) {
                // inside an unit
                currentUiComponent = unitsStack[unitsStackSize - 1];
            } else {
                // inside a page, but outside any unit
                currentUiComponent = runtimeData.currentPage;
            }
        } else {
            // inside the layout
            return "";
        ...
        if (currentZone.addContentProvider(currentUiComponent)) {
            // newly added content
            currentZone.addContentForcefully(currentUiComponent, isOverride, options.fn(this));
        } else {
            if (runtimeData.isProcessingParentUnits) {
                if (currentZone.canAddContent()) {
                    currentZone.addContent(currentUiComponent, isOverride, options.fn(this));
                }
            } else {
                currentZone.addContentForcefully(currentUiComponent, isOverride, options.fn(this));
            }
    ...
    function resourceHelper(type, path, options) {
        ...
        if (zonesStack.length != 1) {
            var msg = "'" + type + "' Handlebars helper should be used inside a top level zone.";
            throw new Error(msg);
        }

        zonesStack[0].addResource(type, (currentUiComponent.fullName + path));
        return "";
    }
        ...
        if (currentZone) {
            ...
            if (currentContentProviderFullName) {
                ...
                if (contents) {
                    if (currentZone.hasSubZones()) {
                        ...
                        var tmpBuffer = [];
                        for (var m = 0; m < numberOfContents; m++) {
                            optionsData._uuf_cci = m;
                            if (options.fn) {
                                tmpBuffer.push(options.fn(this, optionsData));
                            }
                            tmpBuffer.push(contents[m]);
                        }
                ...
                if (currentZone.hasSubZones()) {
                    ...
                    for (var k = 0; k < numberOfContentProviders; k++) {
                        var contentProviderFullName = contentProviders[k].fullName;
                        mainZoneBuffer.push(currentZone.getContents(contentProviderFullName).reverse().join(""));
                    }
            ...
            if (currentZone.hasResources()) {
                var publicUri = renderingData.context.appData.uri
                                + constants.DIRECTORY_APP_UNIT_PUBLIC + "/";
                var resourcesBuffer = [];
                var cssResourcesBuffer = currentZone.getResources("css");
                if (cssResourcesBuffer) {
                    resourcesBuffer.push(['<link rel="stylesheet" href="', publicUri,
                                          cssResourcesBuffer.join(","), "", '">'].join(""));
                }
                var jsResourcesBuffer = currentZone.getResources("js");
                if (jsResourcesBuffer) {
                    resourcesBuffer.push(['<script src="', publicUri, jsResourcesBuffer.join(","),
                                          '"></script>'].join(""));
                }
                // HTML comes after resources. Usually zones with resources do not have HTML. So
                // trim to remove unnecessary spaces and new lines.
                resourcesBuffer.push(zoneHtml.trim());
