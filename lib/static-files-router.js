...
(function () {
     ...
     */
    function getFile(unitFullName, resourceType, fileName, lookUpTable) {
        ...
        if (resourceType == "less") {
            filePathRelativeToUnit = constants.DIRECTORY_APP_UNIT_PUBLIC + "/less/"
                                     + splitFileName(fileName).name + ".less";
        } else {
            filePathRelativeToUnit = constants.DIRECTORY_APP_UNIT_PUBLIC + "/"
                                     + resourceType + "/" + fileName;
        }
        // search in this unit
        var filePath = constants.DIRECTORY_APP_UNITS + "/" + unitFullName + filePathRelativeToUnit;
        ...
        filePath = constants.DIRECTORY_APP_UNITS + "/" + parentUnitFullName
     ...
     * @param response {Object} HTTP response to be served
     */
    function renderLess(lessFile, unitFullName, cssFileName, isCachingEnabled, response) {
        // cached CSS file name pattern: {unitFullName}_{cssFileName}.css
        var cacheFilePath = constants.DIRECTORY_CACHE + "/" + unitFullName + "_" + cssFileName;
    ...
    route = function (request, response, lookUpTable, configs) {
        ...
        var requestedFile = getFile(parts[3], parts[4], parts[5], lookUpTable);
        if (!requestedFile) {
            // this file either does not exists or it is a directory
            var filePath = constants.DIRECTORY_APP_UNITS + "/" + parts[3]
                           + constants.DIRECTORY_APP_UNIT_PUBLIC + "/" + parts[4] + "/"
                           + parts[5];
        ...
        var isCachingEnabled = Boolean(configs[constants.CONF_CACHE_ENABLED]);
        ...
        if (resourceType == "less") {
            ...
            isCachingEnabled = isCachingEnabled && !Boolean(request.getParameter("nocache"));
            renderLess(requestedFile, parts[3], parts[5], isCachingEnabled, response);
        } else {
