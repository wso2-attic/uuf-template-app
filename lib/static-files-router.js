...
(function () {
    ...
    function parseLessResource(lessResource, lessCompiler, isCachingEnabled) {
        ...
        if (isCachingEnabled && cachedFile.isExists() &&
            ...
            // Don't update the cache AND cached file exists AND cached file is newer than the less
    ...
    /**
     * Returns the file specified by the parameters.
     ...
     * @return {?Object} file
     */
    function getFile(unit, resourceType, relativeFilePath, lookupTable) {
     ...
     * @param lookupTable {LookupTable} lookup table
     * @return {{success: boolean, message: string, resources: StaticResource[], hashCode: string}}
     *     resources data
     */
    function getResourcesData(requestedResourcesUris, lookupTable) {
        var rv = {success: true, message: "Success", resources: null, hashCode: null};
        /** @type {StaticResource[]} */
        ...
        for (var i = 0; i < numberOfRequestedResourcesUris; i++) {
            ...
            switch (parts.length) {
                ...
                case 2:
                    ...
                    return rv;
                default:
                    // requestedResource = {unitFullName}/{resourceType}/{+filePath}
                    // For valid URIs : parts.length >= 3
                    // parts = ["unitFullName", "resourceType", ... ]
            ...
            var resourceFile = getFile(providerUnit, resourceType, relativeFilePath, lookupTable);
            if (resourceFile) {
                ...
                rv.success = false;
                rv.message = "Requested resource '" + providerUnit.path + relativeFilePath
    ...
    function getIfModifiedSinceDate(request) {
        var str = request.getHeader("If-Modified-Since");
        return parseInt(str);
    }
    ...
    function setResponseHeaders(file, mimeType, response) {
        ...
        response.addHeader("Last-Modified", String(file.getLastModified()));
    }
    ...
    route = function (request, response, lookupTable, configs) {
        ...
        // get resources data
        var resourcesData = getResourcesData(requestedResourcesUris, lookupTable);
        if (!resourcesData.success) {
            log.warn(resourcesData.message);
            response.sendError(400, resourcesData.message);
            return;
        }
        ...
        var cachingEnabledConfig = parseBoolean(configs[constants.APP_CONF_CACHE_ENABLED], true);
