...
(function () {
     ...
     * @return {Object.<string, UIComponent>} UI components' data
     */
    function getUiComponentsData(componentType, componentsPath) {
        ...
        // Sorting
        uiComponentsList.sort(function (a, b) {
            // 'a' and 'b' will here be two UI Components from the 'uiComponentsList' array.
            ...
            // If their indexes are equal, then compare their full names.
            if (aIndex == bIndex) {
        ...
        for (i = 0; i < numberOfComponents; i++) {
            ...
            var parentComponentFullName = component.definition["extends"];
            while (parentComponentFullName) {
                var parentComponent = uiComponentsMap[parentComponentFullName];
                if (!parentComponent) {
                    var immediateChild = (component.parents.length == 0) ? componentFullName :
                                         component.parents[component.parents.length - 1];
                ...
                component.parents.push(parentComponentFullName);
                componentDefinition = extend(component.definition, parentComponent.definition);
                parentComponentFullName = parentComponent.definition["extends"];
            }
        }
    ...
    getAppConfigurations = function () {
        ...
        return (require(constants.FILE_APP_CONF));
    };
    ...
    getLookupTable = function (configs) {
        ...
        var pagesFullNames = Object.keys(pagesData);
        var numberOfPages = pagesFullNames.length;
        /** @type {Object.<string, string>} */
        var uriPagesMap = {};
        for (var i = 0; i < numberOfPages; i++) {
            var pageFullName = pagesFullNames[i];
            var page = pagesData[pageFullName];
            ...
            var pageUri = page.definition[constants.PAGE_DEFINITION_URI];
            if (uriPagesMap[pageUri]) {
                // A page is already registered for this URI.
                throw new Error("Cannot register page '" + pageFullName + "' for URI '" + pageUri
                                + "' since page '" + uriPagesMap[pageUri]
                                + "' already registered.");
            }
            uriPagesMap[pageUri] = pageFullName;
        }

        // units
        var unitData = getUiComponentsData("unit", constants.DIRECTORY_APP_UNITS);
        var unitsFullNames = Object.keys(unitData);
        var numberOfUnits = unitsFullNames.length;
        /** @type {Object.<string, [string]>} */
        var pushedUnits = {};
        for (var j = 0; j < numberOfUnits; j++) {
            var unitFullName = unitsFullNames[j];
            var unit = unitData[unitFullName];
            ...
            var uriPatterns = unit.definition[constants.UNIT_DEFINITION_PUSHED_URIS];
            if (uriPatterns && Array.isArray(uriPatterns)) {
                var numberOfUriPatterns = uriPatterns.length;
                for (var k = 0; k < numberOfUriPatterns; k++) {
                    var uriPattern = uriPatterns[k];
