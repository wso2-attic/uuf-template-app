...
(function () {
    ...
    function executeScript(operation, event, arguments) {
    ...
    ssoAcs = function () {
        ...
        if (ssoClient.isLogoutResponse(samlResponseObj)) {
            ...
            if (ssoSession.sessionId) {
                ...
                if (!executeScript(OPERATION_LOGIN, EVENT_SUCCESS, getCurrentUser())) {
                ...
                if (relayState) {
                    ...
                    response.sendRedirect(getRedirectUri(OPERATION_LOGIN, EVENT_SUCCESS));
                }
            } else {
    ...
    login = function () {
        ...
        if (isAuthenticated) {
            ...
            response.sendRedirect(getRelayState(OPERATION_LOGIN));
        } else {
            var error = new Error("Incorrect username or password");
            if (!executeScript(OPERATION_LOGIN, EVENT_FAIL, error)) {
                return; // Some error occurred when executing the script.
            }
            var redirectUri = getRedirectUri(OPERATION_LOGIN, EVENT_FAIL) + "?error="
                              + error.message + "&" + constants.URL_PARAM_REFERER + "="
                              + getRelayState(OPERATION_LOGIN);
    ...
    logout = function () {
        ...
        if (!executeScript(OPERATION_LOGOUT, EVENT_SUCCESS, previousUser)) {
            return; // Some error occurred when executing the script.
        }
        response.sendRedirect(getRedirectUri(OPERATION_LOGOUT, EVENT_SUCCESS));
    };
})();